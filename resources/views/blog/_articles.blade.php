...
@if($posts->count() > 0)
		...
		@foreach($posts as $post)
			<article class="group bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 w-full max-w-full">
				...
				<div class="p-3.5 sm:p-4 md:p-5 lg:p-6">
					...
					<h2 class="text-base sm:text-lg md:text-xl lg:text-2xl font-bold text-gray-900 mb-2 sm:mb-2.5 md:mb-3 group-hover:text-teal-700 transition-colors line-clamp-2">
					...
					<div class="flex items-center justify-between pt-2.5 sm:pt-3 md:pt-4 border-t border-gray-100 gap-2 sm:gap-3 md:gap-4">
						<div class="flex items-center gap-1.5 sm:gap-2 min-w-0 flex-1">
							<div class="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 rounded-full bg-teal-100 flex items-center justify-center flex-shrink-0">
								<span class="text-teal-700 text-[10px] sm:text-xs font-semibold">{{ substr($post->author, 0, 2) }}</span>
							</div>
							@if($post->doctor)
								<a href="{{ route('blog.author', $post->doctor) }}" class="text-[11px] sm:text-xs md:text-sm text-gray-600 font-medium hover:text-teal-700 transition-colors truncate">
									{{ $post->author }}
								</a>
							@else
						...
						<a href="{{ route('blog.show', $post) }}" class="inline-flex items-center gap-0.5 sm:gap-1 text-[11px] sm:text-xs md:text-sm font-semibold text-teal-700 hover:text-teal-800 group-hover:gap-1.5 sm:group-hover:gap-2 transition-all flex-shrink-0">
	...
	@endif
@else
	<div class="text-center py-8 sm:py-12 md:py-16 bg-white rounded-lg border border-gray-200" id="blog-empty-state">
